...
In der Motivation (siehe Abschnitt \ref{sec:cryptocurrency}) wurde bereits auf Kryptowährungen allgemein und auf Bitcoin im Speziellen eingegangen. Es wurde ebenfalls angemerkt, dass die zwei Währungen mit dem größten Marktvolumen Bitcoin (BTC) und Etherium (ETH) sind. Aus diesem Grund werden nur die Kurse dieser beiden Währungen analysiert. In diesem Abschnitt wird darauf verzichtet, weiter auf die Technik hinter BTC und ETH einzugehen. Es ist jedoch nicht ausgeschlossen, dass zu einem spätern Zeitpunk Teile des Mining- oder Handelsprozesses genauer betrachtet werden, sollte es dazu beitragen, die Analyse zu verbessern. \todo{so schreiben?}
