...
-wenn man auf probleme stößt, dokumentieren und antworten finden (zb. lange antwortzeiten)

\paragraph{Describe the Data}\mbox{} \\


Anhand des nachfolgenden Beispiels (Datensatz von \citep[Case Lasagne Test.xlsx]{hertle_datenanalyse_2016}) werden die Schritte "Collect the Initial Data" und "Describe the Data" kurz visualisiert. Zuerst werden die Daten eingelesen (Listing \ref{list:LasagneHead}) und anschließend oberflächlich betrachtet (Struktur in Tabellen \ref{tab:LasagneHead:output} und Datentypen in Abbildung \ref{fig:Lasagne_RStudio}).
...
\begin{table}[h!]
\centering
\begin{tabular}{p{1cm}p{0.7cm}p{1.1cm}p{1.8cm}p{1.6cm}p{1.6cm}p{3.6cm}p{1.8cm}}
  \hline
...
\hline
\end{tabular}
\begin{tabular}{p{2cm}p{2cm}p{5.7cm}p{3cm}}
