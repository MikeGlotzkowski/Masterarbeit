...
\subsection{Determine the Data Mining Goals}\label{subsec:goals}
\begin{longtable}[!h]{|p{4,5cm}|p{12cm}|}
\hline
\textbf{Output} & \textbf{Beschreibung} \\ 
\hhline{==}
...
\subsection{Produce a Project Plan} \label{subsec:projectPlan}
...
Betrachtet man zuerst die beiden Regressionen (Tabellen \ref{tab:BTCReg} und \ref{tab:ETHREg}), so fällt auf, dass $ R^2 $ sehr hoch ist (Maximum ist $ R^2 = 1 $). Auf den ersten Blick zeugt das von einer nahezu perfekten Kurspreisvorhersage. Untersucht man das Ergebnis genauer, so sieht man, dass die vorhergesagten Werte sehr nahe an den tatsächlichen liegen (siehe Tabelle \ref{tab:scoredReal}).
\begin{table}[H]
\centering
...
\label{tab:BTCReg}
\end{table}
Bei der Beschreibung der Bewertungsmetrinken in Punkt \ref{subsec:goals} wurde jedoch darauf hingewiesen, dass große Werte für $ R^2 $ misstrauisch machen sollten. Dieses Phänomen kann mehre Gründe haben. Hier liegen folgende Vermutungen vor:
\begin{itemize}
...
\item Wenn zu viele Variablen im Verhältnis zu den Beobachtungen vorliegen, kann es auch zu einer Zufallskorrelation (engl. chance correlation) kommen.\citep{lohninger_teach/me_1999}
...
Nach der Ergebnisbetrachtung werden diese nun hinsichtlich der business objectives und der business success criteria betrachtet.
