...
\begin{longtable}[H]{|p{6cm}|p{9,5cm}|}
\hline
\textbf{Output} & \textbf{Beschreibung}\\
\hhline{==}
...
Um nur die Zeilen zur Analyse zu benutzen, die im ausgewählten Zeitintervall (siehe xxx) liegen, wird das Datensetz noch aufgesplittet (Experiment Item 'Split Data' mit Relative Expression \textbackslash "Date" >= 2011-01-01T00:00:00). Daraufhin werden mit dem Item 'Clean Missing Data (Probabilistic PCA))' fehlende Daten ersetzt. Bei neun Spalten ist dies nicht möglich (keine Fehlermeldung). Diese werden mit einem erneuten 'Clean Missing Data (Remove entire column)' entfernt. Als Abschluss wird mit dem R-Script in Listing \ref{list:AzureIncrease} eine neue Spalte eingeführt, die angibt, ob der Kurs zum Vortag gesteigen ist (Wert: 1) oder nicht (Wert: 0). Dieser wird im Nachfolgenden als Vorhersagewert genutzt.
...
alles supervised, weil ich ziel label habe.
einmal genauen preis (continuous values) "regressions" und einmal two class für 1: gestiegen und 0 für nicht gesteigen; wäre auich denkbar cluster mit "leicht fallend, leicht steigend" etc, aber zu aufwändig.

in azure: 
two class aufzählen
regressionen aufzählen -ordinary regression -poison regression -linear regression -quantil (eignet sich gut, aber andere form)

alle parametrisierbar. hier standard parameter


\subsection{Generate Test Design}


muss überlegt werden und kann für versch. algos anders sein. es gibt auch ansätze, nicht verhältnismäßig zu splitten, sondern auch algos dafür. hier aber klassische 80:20 mit dem split modul mit random (nicht einfach obere/unterere hälfte).
...
model aufschreiben und welche parameter "model description"

sind quasi 2: 2class beschreibung und regression beschreibung

2class:
Algorithmus Accuracy Precision Recall F1-Score AUC Average Log Loss
...
hier r modul und add rows --> übersuchtlich in tabelle schreiben
ranking schwer, da unterschiedl.

ranking bei "nicht-treffern" sinnlos, deswegen nur 2class regressionen
