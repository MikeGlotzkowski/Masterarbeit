...
\subsection{Aufstellung der Projetressourcen und Einflussfaktoren (Assess the Situation)} \label{subsec:assesTheSituation}
...
\begin{centering} \begin{longtable}[H]{|p{6,5cm}|p{10cm}|}
\hline
\textbf{Output} & \textbf{Beschreibung} \\ 
\hhline{==}
...
\end{itemize}
Die Auswahl fällte auf den F1-Score für Klassifikationen, da er Recall und Precision inkludiert und im Gegensatz zum AUC zwischen mehreren Modellen direkt vergleichbar ist (Maximalwert 1 statt nach oben offen). Bei der Regression fällt die Wahl auf $ R^2 $, da sie Modelle ebenfalls direkt vergleichbar macht.
...
\begin{centering} \footnotesize \begin{longtable}[!h]{|p{4cm}|p{4cm}|p{4cm}|}
\hline
...
\section{Data Preperation}\label{sec:p3}
...
\label{tab:selecData}
\end{longtable} \end{centering}\lstinputlisting[caption=Konvertierung von pseudo-numerischen Werten,label=list:pseudonumeric]{../R/pseudonumeric.R}
...
Würde man versuchen, einen Münzwurf mit einem mathematischen Modell vorherzusagen (was nicht möglich ist), so würde die statistische Genauigkeit (accuracy) bei 50\% liegen. In Tabelle \ref{tab:ETH2} und \ref{tab:BTC2} ist zu sehen, dass die Accuracy nicht deutlich über diesem Wert liegt. Bei einigen Algorithmen ist jedoch ein Wert von ca. 60\% (0.6) erkennbar.
...
\label{tab:BTCBest}
\end{table}
Anzumerken ist hier die interessante Tatsache, dass der Kurs sehr genau vorhergesagt wurde (wenn auch durch Fehler in Modell), eine Aussage darüber, ob der Kurs steigt oder nicht, jedoch kaum möglich ist. Der Bitcoinkurs scheint jedoch stärker von den Faktoren beeinflusst zu werden (höherer F1-Score).
